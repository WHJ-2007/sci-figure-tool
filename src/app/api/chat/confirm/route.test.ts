...
describe("POST /api/chat/confirm", () => {
  it("多挂起项分两批确认：首批确认后会话保留，次批补全后会话删除", async () => {
    ...
    // 构造器把元素复制为 initialIds：之后 deleteElement 才会挂起等待确认（AI 本轮创建的直接删）
    const d = new DraftCanvas([elA, elB]);
    d.deleteElement({ id: "a1" });
    d.deleteElement({ id: "b1" });
    expect(d.pending.length).toBe(2);
    const sid = crypto.randomUUID();
    setConfirmSession(sid, d);
    // 首批：确认 a1，取消 b1（取消也算表态，但本批有新表态 → 完成批不删会话）
    const res1 = await POST(
      mockReq({ sessionId: sid, approvals: [{ id: "a1", approved: true }, { id: "b1", approved: false }] })
    );
    expect(res1.status).toBe(200);
    const ev1 = await readEvents(res1);
    expect(ev1.some((e) => e.type === "snapshot" && e.canvas.elements.length === 1)).toBe(true);
    // 会话仍在（b1 尚未确认执行）
  ...
  it("重复提交同一批次幂等：不重复应用", async () => {
    const elA = makeElement("rect", 0, 0, 100, 60, { id: "a1" });
    const d = new DraftCanvas([elA]);
    d.deleteElement({ id: "a1" });
    const sid = crypto.randomUUID();
    setConfirmSession(sid, d);
    const res = await POST(mockReq({ sessionId: sid, approvals: [{ id: "a1", approved: true }] }));
    await readEvents(res);
    // 幂等重试同一 POST（模拟网络重发）：完成批自身不删会话，仍 200
    const res2 = await POST(mockReq({ sessionId: sid, approvals: [{ id: "a1", approved: true }] }));
    expect(res2.status).toBe(200);
    const ev2 = await readEvents(res2);
  ...
  it("伪造挂起项 id 不计入已表态：不会虚增 resolved 提前删会话", async () => {
    ...
    // 过滤后 evil 从未计入 resolved → 重试批仍不算"全部表态"，会话保留等待 a1
    expect(getConfirmSession(sid)).toBeDefined();
  });
