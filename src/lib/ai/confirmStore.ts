...
export function markResolved(sessionId: string, ids: string[]) {
  const s = sessions.get(sessionId);
  if (!s) return;
  for (const pid of ids) if (typeof pid === "string") s.resolved.add(pid);
}
...
export function isSessionComplete(sessionId: string, pendingCount: number): boolean {
  const s = sessions.get(sessionId);
  if (!s) return true;
  if (s.resolved.size >= pendingCount) {
