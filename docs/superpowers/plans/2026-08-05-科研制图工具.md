...
| Task 36 Shift 多选 | ✅ 完成 | bffd02a 实现（Shift+点击 toggle 加入/移出选区；Shift+空白框选追加到现有选区；多选组可整体拖动） |

**当前验证基线**：`npm test` = 20 文件 153 测试全绿（tsc --noEmit + vitest）；`npm run build` 通过；`tools/launcher/test-core.ps1` 11/11、`test-launcher.ps1` 4/4 全绿（2026-08-05 Task 20 收尾时点）。
