...
> 环境准备：jsdom 缺 `Element.prototype.scrollTo`（组件 useEffect 自动滚动用）与 `Blob.prototype.stream`（mockStream 用），需在 `src/test/setup.ts` 的 window 守卫块内补两个桩（scrollTo 直接置空函数；Blob.stream 用 FileReader 读出内容再包装成 ReadableStream，加 `Uint8Array<ArrayBuffer>` 泛型满足 TS strict）。与既有 setPointerCapture/getBoundingClientRect 桩同类，滚动/流语义由真实浏览器保证。
