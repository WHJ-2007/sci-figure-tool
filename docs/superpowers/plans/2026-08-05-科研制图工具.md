...
> 若 `vi.mock` 后 `import { generateText } from "ai"` 在 agent.ts 内被拦截，mock 需 `vi.mock("ai", () => ({ generateText: vi.fn() }))` 且测试内 `const mocked = vi.mocked(await import("ai")).generateText`。以测试全绿为准。
