...
| Task 16 聊天面板与生成流 | ⚠️ 实现完成 + spec ✅ + quality ✅（5 条 minor 已修） | **续接点：对 b7cf95c 的修复做一次 quality 复审（提交 ~10s），通过后本任务关闭** |
| Task 17 一键生成入口与引导 | ⬜ 未开始 | 计划行号约 3539+（随文档编辑有偏移，按 ### Task 17 搜索） |
