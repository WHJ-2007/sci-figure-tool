...
历史策略：**快照在每次 set 时与文档一起写入**（一个 set 回调内同时返回 `doc` 与 `history`）。交互类高频动作（拖动/缩放/旋转的逐帧更新）走不带历史的 `updateElementFast` / `moveElements`，手势结束时由交互层调用 `commitHistory()` 只推一次快照。
  ...
  const onPointerMove = useCallback(
    (e: React.PointerEvent<SVGSVGElement>) => {
      ...
      if (m.kind === "move") {
        ...
        // moveElements 不入历史，pointerup 时一次 commitHistory
...
> 说明：`moveElements` 不带历史（高频调用），pointerup 时 `commitHistory()` 只推一次快照，一次拖动 = 一步撤销。
...
- **历史策略**：带历史的动作（add/update/delete/setDoc）在单次 set 内同时写 doc 与 history；高频逐帧动作（`updateElementFast`、`moveElements`）不入历史，手势结束由交互层 `commitHistory()` 提交一次。一次拖动/缩放/旋转 = 一步撤销。
