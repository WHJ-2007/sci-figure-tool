...
| Task 39 玻璃边缘 + 图案/逻辑气泡 + AI 逻辑规范 | ✅ 完成 | 062ddc8 实现（① 画布玻璃边缘：svg 外层 .glass-canvas 玻璃面板（半透明渐变 + backdrop-blur + 内高光描边 + 外投影 + 圆角，globals.css），svg 内白色背景 rect（data-testid=canvas-bg、pointer-events none），画布居中；② 工具栏整合：常驻选择/小手 + 图案主按钮（图标 = 当前子工具，气泡含矩形/圆角/椭圆/三角/菱形/六边形/箭头/折线/文字）+ 逻辑主按钮（气泡含逻辑节点）；气泡非阻塞（点击主按钮 toggle、点外 pointerdown 关闭、气泡内连续切换工具不关闭）；③ AI skill：prompt 规则 12/13 + createElement 描述——语义模块必须用 logic 禁 rect+文字两件套、标题 ≤8 字简洁语义名词、宽高自动扩框无需精确指定、同一流程统一尺寸对齐间距 20~60px、connectElements 锚点自动对接。新增 9 测试（Toolbar 气泡 7 + 玻璃边 2） |
| Task 40 WASD 平移视口 | ✅ 完成 | 11afe4b 实现（W/A/S/D 每次平移 view.ox/oy 50px、按住 keydown 自动重复持续移动；仅移动视图不影响元素/选区；输入框/文字编辑中、AI 生成中、Ctrl/⌘ 组合键不触发——Ctrl+W 保留系统快捷键；新增 5 测试）。另处理了端口占用：残留的 dev server 进程（PID 31844，占 3001）已终止，3000/3001 均空闲 |
| Task 41 气泡空白修复 + WASD 方向修正 + 生成中平移 + 逻辑正文 | ✅ 完成 | 1490ec9 实现（① 气泡"空的"：toolbar backdrop-blur 创建 stacking context，气泡 z-30 被锁内部、被画布区盖住——toolbar 容器加 relative z-40；② WASD 方向按用户期望调整（W=上 A=左 S=下 D=右，oy/ox 符号取反）；③ AI 生成中 WASD 放行（isGenerating 检查移到 WASD 分支之后）；④ 逻辑节点正文：body 多行（\\n 分隔，正文字号自动小 2 号），logicBoxSize 按标题+正文统一算框尺寸，store/draft 双路径 updateElement 检测 body 并自动扩框，ElementShape/PropertyPanel（正文 textarea）/exporter 全链路支持，AI createElement/updateElement 支持 body + prompt 引导要点式正文。新增 6 测试（elements 尺寸、store 扩高、agent create/update、Canvas 渲染 3 text 正文小字号、exporter 正文）） |
| Task 42 逻辑触点拉箭头 + AI 简洁回答 | ✅ 完成 | 1490ec9 实现（① 选中逻辑节点时 4 锚点圆点变为可拖动触点（data-anchor + pointerEvents all + crosshair），按住触点直接拖出箭头——复用 draw-line arrow 模式（起点=锚点坐标），拖动中终点吸附其他逻辑节点锚点（12px，nearestAnchor 增加 excludeElementId 排除源节点自身），松手创建精确连接（startId/endId 反查）；触点 stopPropagation 不影响本体拖动/缩放；② prompt 规则 11 强化：一两句大白话总结画了什么，不罗列坐标/尺寸/工具调用。新增 3 测试（触点拖箭头吸附连接、不吸附回源、本体拖动不受影响）） |
| Task 43 右侧面板亚克力极简风 | ✅ 完成 | 1490ec9 实现（右侧改两张独立悬浮亚克力卡片：.glass-panel（圆角 18 + blur 24px saturate 1.5 + 内高光 + 外投影，与 .glass-canvas 同语言）+ 页布局 gap-3 p-3；PropertyPanel 控件磨砂化（圆角/白描边/白色输入/蓝色 focus 柔光、B/I/对齐激活态蓝底）；ChatPanel 气泡圆角胶囊（rounded-2xl、用户蓝 / AI 白）、输入区圆角卡片、发送按钮圆角胶囊；页面背景改柔和三色渐变（#e2edfb → #f2efff → #e6f3ef）衬托通透质感） |
| Task 44 AI 一键布局（applyGraph + dagre）+ prompt 科研审美规范 | ✅ 完成 | c0a4422 实现（① GitHub 调研（PaperBanana/AutoFigure/DeepDiagram/AIGP/d2/Mermaid 6 项目）结论：语义与布局分离是行业共识——AI 只声明 nodes+edges，坐标交给布局引擎；选 dagre（Sugiyama 分层布局，Mermaid/React Flow/d2 同款，轻量纯前端离线）而非 elkjs（1MB 复杂）或自研；② 新增 graphLayout.ts 布局模块（nodesep/ranksep 60、TB/LR、返回左上角、超出画布自动缩放——位置整体缩放节点尺寸不变，预算扣除最大节点尺寸防底部越界，最低 0.5 倍）+ 6 测试；③ DraftCanvas.applyGraph：节点尺寸按 logicBoxSize 估算、edges 引用校验（无效引用报错且不创建任何元素）、logic 节点创建 + connectElements 连线 + 活动日志；④ tools.ts 注册 applyGraph（nodes 声明 id/text/body/fill + edges 关系 + direction TB/LR）；⑤ prompt 规则 13 改为 applyGraph 优先、新增规则 14 科研审美规范（受控调色板 #eef4ff/#f0fff0/#fff8e6/#f3efff/#ffeef0/白，同图 ≤3 种、边框统一 #2f2f2f、论文插图式克制配色）。新增 10 测试（graphLayout 6 + applyGraph 4）。依赖：dagre + @types/dagre） |

**当前验证基线**：`npm test` = 21 文件 212 测试全绿（tsc --noEmit + vitest）；`npm run dev` 编译通过（2026-08-05 Task 44 收尾时点）。launcher 测试上次基线 11/11、4/4（Task 40 后未改动 launcher）。
