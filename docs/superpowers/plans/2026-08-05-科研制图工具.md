...
| Task 36 Shift 多选 | ✅ 完成 | bffd02a 实现（Shift+点击追加选区；Shift+空白框选追加到现有选区；多选组可整体拖动）；7c8eeec 修复（Shift 点击已选元素后拖动乱动——toggle 移除会让被点击元素原地不动、别的元素在动；改为点击已选元素保持选区、Shift 只追加不移除，取消选择走空白点击清空） |

**当前验证基线**：`npm test` = 20 文件 154 测试全绿（tsc --noEmit + vitest）；`npm run build` 通过；`tools/launcher/test-core.ps1` 11/11、`test-launcher.ps1` 4/4 全绿（2026-08-05 Task 20 收尾时点）。
