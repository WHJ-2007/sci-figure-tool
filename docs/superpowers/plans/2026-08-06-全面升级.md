...
| 7 图表数据表编辑 | ✅ | 4d146f9 |
| 8 操作气泡时间线 | ✅ | 3b7f73f |
| 9 科研绘图 prompt 质量 | ✅ | c5e7780 |
| 10 收尾 | ✅ | 3f03f0e |

验证基线：`npm test` = 26 文件 290 测试全绿 + `npx next build` 编译通过。
...
- **Task 4 持久化语义修正**：`selectMode` 全部具体模式取消后原本持久化 `"[]"`，与刷新恢复语义（auto 状态统一存 "auto"）不一致 → 改为取消全部后存 `"auto"`。
- **Task 5 测试定位方式**：计划强制 `{icon}<span>label</span>` 按钮文案结构，`getByText` 返回 span 而非 button → 断言 aria-pressed 改用 `.closest("button")!` 定位。
- **Task 6 两处**：① `TYPE_NAMES` 类型须为 `Record<ElementType | "rounded", string>`（含 "rounded" 键，计划只写了 ElementType）；② 边框色（stroke）颜色输入在重设计中被移除——计划明确只要求填充预设色板 + 自定义取色器 + 滑块，此为遵循计划的取舍（边框统一深色由科研审美规范承担）。
- **Task 8 三处**：① jsdom 中 `fireEvent.click` 事件向上冒泡，计划给的测试（click 内部 button）不可行 → click handler 移到外层 testId div（role="button" + tabIndex + aria-expanded）；② 展开时重复渲染最新条目导致 getByText 抛 multiple → 展开列表渲染 `shown.slice(0, -1)`；③ 活动文案断言修正：`titleOf` 对无文字元素返回类型名、不带书名号（断言为「修改矩形：填充色」）。
- **Task 9 颜色上限矛盾修复**：计划文本「同图 ≤3 种颜色（2~4 个语义色 + 至多 1 个强调色）」内部矛盾（上限达 5 色）→ 修复为「同图 ≤3 个语义色 + 至多 1 个强调色（总计 ≤4 色）」，与规格一致；测试敏感性：`20~60` 与 `#eef4ff` 在改动前已存在，增强断言 `48~60` 与「总计 ≤4 色」。
- **Task 10 changelog 免追加**：计划的 9 条 changelog 已由各任务 implementer 先行追加（另含 1 条活动文案人话化），核对全覆盖，无需再补。
